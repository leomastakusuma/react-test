...
var TodoApp = React.createClass({
	...
	observe(props, state) {
		return {
			todos: new ReactRethinkdb.QueryRequest({
				query: r.table('tv_shows').orderBy({index: 'createdAt'}),
				changes: true,
				initial: []
			}),
		};
	...
	handleNewTodoKeyDown: function (event) {
		...
		if (val) {
			var q = r.table('tv_shows').insert({title: val, completed: false, createdAt: r.now()});
	...
	toggleAll: function (event) {
		...
		var q = r.table('tv_shows').update({completed: checked});
		RethinkSession.runQuery(q);
	},

	toggle: function (todoToToggle) {
		var q = r.table('tv_shows').get(todoToToggle.id).update({completed: r.row('completed').not()});
		RethinkSession.runQuery(q);
	},

	destroy: function (todo) {
		var q = r.table('tv_shows').get(todo.id).delete();
		RethinkSession.runQuery(q);
	},
	...
	save: function (todoToSave, text) {
		var q = r.table('tv_shows').get(todoToSave.id).update({title: text});
		RethinkSession.runQuery(q);
	...
	clearCompleted: function () {
		var q = r.table('tv_shows').filter({completed: true}).delete();
		RethinkSession.runQuery(q);
	},
